...
public class QuestionsController
{
    ...
    protected void initBinder(final WebDataBinder webDataBinder)
    {
        ...
        webDataBinder.addValidators(gapFillingQuestionValidator);
    }
    ...
    public String getBookQuestionsView(@AuthenticationPrincipal User user, @PathVariable Long bookId, ModelMap modelMap)
    {
        ...
        if (optionalBook.isPresent() && optionalBook.get().getUser().getId() == user.getId())
        {
            ...
            modelMap.put("gapFillingQuestions", optionalBook.get().getGapFillingQuestions());
            return "questions";
        } else
        {
            //todo: show errors, no match found when book is empty
            System.out.println("BookId problem");
            return "redirect:/books";
        }
    }

    @GetMapping("/new")
    public String createNewQuestionView(@AuthenticationPrincipal User user, @PathVariable Long bookId, ModelMap modelMap)
    {
        Optional<Book> optionalBook = bookService.findById(bookId);
        if (optionalBook.isPresent() && optionalBook.get().getUser().getId() == user.getId())
        {
            modelMap.put("saveLink", "/books/" + bookId + "/questions/new/save");
            modelMap.put("saveAndNewLink", "/books/" + bookId + "/questions/new/saveAndNew");
            return "newQuestion";
        }
        else
        {
    ...
    public String getQuestionView(@AuthenticationPrincipal User user, @PathVariable Long bookId, @PathVariable Long questionId, ModelMap modelMap)
    {
        Optional<GapFillingQuestion> optionalGapFillingQuestion = gapFillingQuestionService.getById(questionId);
        Optional<Book> optionalBook = bookService.findById(bookId);

        if (optionalGapFillingQuestion.isPresent() && optionalBook.isPresent() && optionalBook.get().getUser().getId() == user.getId())
        {
            modelMap.put("saveLink", "/books/" + bookId + "/questions/new/save");
            modelMap.put("saveAndNewLink", "/books/" + bookId + "/questions/new/saveAndNew");
            modelMap.put("gapFillingQuestion", optionalGapFillingQuestion.get());
            return "newQuestion";
        } else
        {
            //todo: no question found for edit
            System.out.println("error there is no such a question");
            return "redirect:/books";
        }
    }
    ...
    @PostMapping(value = {"/new/save", "/new/saveAndNew"})
    public String createNewQuestion(@AuthenticationPrincipal User user, @PathVariable Long bookId, @Valid GapFillingQuestion gapFillingQuestion, BindingResult bindingResult, RedirectAttributes redirectAttributes, HttpServletRequest httpServletRequest)
    {
        ...
        if (bindingResult.hasErrors())
        {
            ...
            return "redirect:/books/" + bookId + "/questions/new";
        }
    ...
    public String deleteQuestion(@AuthenticationPrincipal User user, @PathVariable Long bookId, @PathVariable Long questionId, RedirectAttributes redirectAttributes)
    {
        List<ToastMessage> toastMessages = new ArrayList<>();
        redirectAttributes.addFlashAttribute("toastMessages", toastMessages);
        ...
        Optional<GapFillingQuestion> gapFillingQuestionOptional = gapFillingQuestionService.getById(questionId);

        if (gapFillingQuestionOptional.isPresent())
        {
            gapFillingQuestionService.delete(questionId);
